...
def run(data_params: Dict[str, Any], 
    ...
    if model_params["solver"] == "qplayer":
        is_robust_l, obj_l, obj_bd_l, opt_status_l = [0], [0], [0], [0]
    else:
        assert False
    ...
    return dict(
        ...
        y_true_cls = (y[idx_test] * 2 - 1).numpy(force=True).tolist()[0],
        y_pred_logit = y_pred.numpy(force=True).tolist()[0],
        y_worst_obj = obj_l[0],
        y_worst_obj_bound = obj_bd_l[0],
        y_is_robust = is_robust_l[0],
        y_opt_status = opt_status_l[0],
        # split statistic[0s
