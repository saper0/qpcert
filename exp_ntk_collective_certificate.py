...
def run(data_params: Dict[str, Any], 
    ...
    svm_alpha = ntk.svm
    obj, is_robust, y_worst_obj, opt_status = utils.certify_collective_bilevel_svm(idx_labeled, idx_test, ntk_test, ntk_lb, ntk_ub, y, y_pred,
            svm_alpha, C=model_params["regularizer"], M=1e3, Mprime=1e3)
    n_robust_frac = (y_pred.shape[0]-obj) / y_pred.shape[0]
    # acc_cert_u = 0 #not implemented
    logging.info(f"Certified #nodes (poisoning): {n_robust_frac}")
    ...
    return dict(
        ...
        cert_node_frac = n_robust_frac,
        cert_status = opt_status,
        ...
        y_worst_obj = y_worst_obj.tolist(),
        y_is_robust = is_robust.tolist(),
