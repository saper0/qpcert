...
class DC_CSBM(GraphGenerationModel):
    ...
    def __init__(self, n: int, avg_within_class_degree: float, 
        ...
        self.mu = np.array([K*sigma / (2 * self.d**0.5) for i in range(self.d)], 
                           dtype=np.float32)
        self.deg_correction = np.random.uniform(0,1,n)
        print("DC_CSBM mu:")
        print(self.mu)
    ...
    def sample(self, n: int, seed = 0) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        for i in range(n):
            for j in range(i+1, n):
                if y[i] == y[j]:
                    edge_prob[i,j] = self.p*self.deg_correction[i]*self.deg_correction[j]
                else:
                    edge_prob[i,j] = self.q*self.deg_correction[i]*self.deg_correction[j]
