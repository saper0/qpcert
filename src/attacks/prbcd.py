...
class PRBCD(SparseAttack):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__(**kwargs) # set self.edge_index, self.edge_weight (=1) from adj proviced in **kwargs 
        ...
        self.current_search_space: torch.Tensor = None #Indices if Adj. would be one long list
        self.modified_edge_index: torch.Tensor = None #Indices for Adj. as 2D-matrix
        self.perturbed_edge_weight: torch.Tensor = None #P
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            with torch.no_grad():
                ...
                if epoch < self.epochs_resampling - 1:
                    ...
                    self.current_search_space = best_search_space.detach().clone().to(self.device)
                    self.modified_edge_index = best_edge_index.detach().clone().to(self.device)
                    self.perturbed_edge_weight = best_edge_weight_diff.detach().clone().to(self.device)
                    self.perturbed_edge_weight.requires_grad = True
    ...
    def get_modified_adj(self):
        ...
        # Allow removal of edges; edge_weight = 1 - eps or eps
    ...
    def sample_random_block(self, n_perturbations: int = 0):
        """Samples the indices to perturb in A and saves them in [row_idx, col_idx]
        format in self.modified_edge_index.

        Note: Samples with replacement and then only takes the unique indices,
              thus the number of truly chosen elements will be <= b.

        Args:
            n_perturbations (int, optional): Global Attack Budget. Defaults to 0.

        Raises:
            RuntimeError: If the sampled search space is smaller then n_perturbations.
        """
        for _ in range(self.max_final_samples):
            ...
            # edge_weight must be at least epsilon, such that adjacency element is not lost in sparse matrix format
