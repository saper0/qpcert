...
class GlobalStructureAttack(ABC):
...
#@typechecked
class StructureAttack(GlobalStructureAttack):
    """
    ...
    def project(n_perturbations: int, values: torch.Tensor, eps: float = 0, inplace: bool = False):
        ...
        if torch.clamp(values, 0, 1).sum() > n_perturbations:
            left = (values - 1).min()
            right = values.max()
            miu = StructureAttack.bisection(values, left, right, n_perturbations)
...
#@typechecked
class SparseStructureAttack(StructureAttack):
    """
