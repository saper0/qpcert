...
def run(data_params: Dict[str, Any], 
    ...
    linf = torch.linalg.vector_norm(X, ord=torch.inf, dim=1)
    print(f"l2 mean: {l2.mean():.4f}")
    print(f"l2 std: {l2.std():.4f}")
    print(f"l2 min: {l2.min():.4f}")
    print(f"l2 max: {l2.max():.4f}")
    print(f"linf mean: {linf.mean():.4f}")
    print(f"linf std: {linf.std():.4f}")
    print(f"linf min: {linf.min():.4f}")
    print(f"linf max: {linf.max():.4f}")
    l2_pair = torch.cdist(X, X, p=2)
    print(f"l2_pair mean: {l2_pair.mean():.4f}")
    print(f"l2_pair std: {l2_pair.std():.4f}")
    print(f"l2_pair min: {l2_pair.min():.4f}")
    print(f"l2_pair max: {l2_pair.max():.4f}")
    for k in range(n_classes):
        print(f"Class {k} Details:")
        y_mask_k = y == k
        for j in range(n_classes):
            y_mask_j = y == j
            l2_pair_thisclass = l2_pair[y_mask_k, :]
            l2_pair_thisclass = l2_pair_thisclass[:,y_mask_j]
            print(f"l2_pair to class {j} mean: {l2_pair_thisclass.mean():.4f}")
            print(f"l2_pair to class {j} std: {l2_pair_thisclass.std():.4f}")
            print(f"l2_pair to class {j} min: {l2_pair_thisclass.min():.4f}")
            print(f"l2_pair to class {j} max: {l2_pair_thisclass.max():.4f}")
    
    linf_pair = torch.cdist(X, X, p=torch.inf)
    print(f"linf_pair mean: {linf_pair.mean():.4f}")
    print(f"linf_pair std: {linf_pair.std():.4f}")
    print(f"linf_pair min: {linf_pair.min():.4f}")
    print(f"linf_pair max: {linf_pair.max():.4f}")
    for k in range(n_classes):
        print(f"Class {k} Details:")
        y_mask_k = y == k
        for j in range(n_classes):
            y_mask_j = y == j
            linf_pair_thisclass = linf_pair[y_mask_k, :]
            linf_pair_thisclass = linf_pair_thisclass[:,y_mask_j]
            print(f"linf_pair to class {j} mean: {linf_pair_thisclass.mean():.4f}")
            print(f"linf_pair to class {j} std: {linf_pair_thisclass.std():.4f}")
            print(f"linf_pair to class {j} min: {linf_pair_thisclass.min():.4f}")
            print(f"linf_pair to class {j} max: {linf_pair_thisclass.max():.4f}")
    ...
    with torch.no_grad():
        ...
        y_pred_trn, _ = ntk(idx_labeled=idx_labeled, idx_test=idx_labeled,
                               y_test=y, X_test=X, A_test=A, return_ntk=True)
        topk = y_pred.topk(2).values
        diff = topk[:,0] - topk[:,1]
        rel_diff = torch.abs(diff / topk[:,0])
        diff_str = [f"{el:.4f}" for el in diff]
        rel_diff_str = [f"{el:.2f}" for el in rel_diff]
        ...
        print(f"Avg. Diff.: {diff.mean()}")
        print(f"Std. Diff.: {diff.std()}")
        print(f"Min Diff.: {diff.min()}")
        print(f"Max Diff.: {diff.max()}")
        print(f"Avg. Rel. Diff.: {rel_diff.mean()}")
        print(f"Std. Rel. Diff.: {rel_diff.std()}")
        print(f"Min Rel. Diff.: {rel_diff.min()}")
        print(f"Max Rel. Diff.: {rel_diff.max()}")
        print(f"Differences: {diff_str}")
        print(f"Rel. Differences: {rel_diff_str}")
        assert False
